...
namespace Gestion_Hotel
{
    public partial class ConfirmDeleteRes : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            using (NpgsqlConnection c = new NpgsqlConnection("server=localhost; port=5432;user id=postgres; password=rabab; database=GestionHotel"))
            {
                ...
                using (NpgsqlConnection cnx = new NpgsqlConnection("server=localhost; port=5432;user id=postgres; password=rabab; database=GestionHotel"))
                {
                    if (!cnx.State.Equals(ConnectionState.Open))
                        cnx.Open();
                    //MessageBox.Show(idCli + "--" + idRoom);
                    string rq = "update public.\"reservation\" set \"dateFinRes\"='" + DateTime.UtcNow + "', \"liberer\"='" + true + "' where \"idClient\" = '" + idCli + "' and  \"idChambre\"='" + idRoom + "' ";
